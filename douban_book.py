...
class BookList:
    ...
    def main(self):
        ...
        for i in range(start_num, end_num, 20):
            url_1 = self.scrape.douban_book_page_url(url, i)
            soup = self.scrape.get_html_text(url_1)
            name_list = self.data.get_book_name(soup, name)
            point_list = self.data.get_douban_point(soup, points)
            comment_list = self.data.get_comment_number(soup, comment_num)
            url_list = self.data.get_douban_book_urls(soup, urls)
            # price_list = self.data.get_price(soup, price)

            self.view.show_process(i, end_num)

        self.view.make_book_exexl(name_list,comment_list, url_list)
